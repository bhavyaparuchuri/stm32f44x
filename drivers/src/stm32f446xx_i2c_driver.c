...
void I2C_Init(I2C_Handle_t *pI2CHandle)
{
	...
	//3. Configure the device address (Application when device is slave)
	//4. Enable the Acking
	//5. Configure the rise time for I2C pins

	/*
	 * NOTE: All the above configurations must be done when the peripheral is disabled in the control register
	 */
	uint32_t tempreg = 0;
	uint8_t trise;
	...
	pI2CHandle->pI2Cx->CR1 = (tempreg & 0x3F);
 ...
 *
 * @brief						-	This function sent the value of given GPIO port
 *
 * @param[in]					-	Base address of the gpio port
 * @param[in]					-
 * @param[in]					-
...
void I2C_MasterSendData(I2C_Handle_t *pI2CHandle,uint8_t *pTxbuffer, uint32_t Len, uint8_t SlaveAddr)
{
	...
	I2C_GenerateStartCondition(pI2CHandle->pI2Cx);
		...
		//2. confirm that start generation is completed by checking the flag in the SR1
	//NOTE: Untill SB is cleared SCL will be stretched (pulled to LOW)
	while(!	I2C_GetFlagStatus(pI2CHandle->pI2Cx,I2C_FLAG_SB)	);

	//3. Send the address of the slave with r/nw bit set to w(O) (Total 8 bits)
	...
	//when BTF=1 SCL will be stretched (pulled to LOW)
		   ...
		   while(! I2C_GetFlagStatus(pI2CHandle->pI2Cx, I2C_FLAG_BTF) );//wait till TXE is set
...
void I2C_ManageAcking(I2C_RegDef_t *pI2Cx, uint32_t EnorDi)
{
		...
		if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
				{
					...
					if(pI2CHandle->TxRxState == (1 << I2C_SR1_TXE) )
					{
						//BTF, TXE = 1
						if(pI2CHandle->TxLen == 0)
						{
